...
class BookTest < ActionDispatch::IntegrationTest
  ...
  test "book_administration" do
    ...
    new_book_ruby = george.add_book :tags => 'Ruby, Programming', :book => {
    ...
    george.edit_book new_book_ruby, :tags => 'Object-oriented Programming', :book => {
  ...
  module BookTestDSL
    ...
    def add_book(parameters)
      ...
      parameters_tags_list = parameters[:tags].sub(', ', ',').split(',')
      assert_equal parameters_tags_list.size, book.tags.size
      for tag_name in parameters_tags_list do
        assert_equal 1, Book.tagged_with(tag_name, :any => true).size
      end
      return book;
    end
